...
(function ($) {
  "use strict";
  $.fn.josa = function (josaInput) {
    // "이/가", "을/를", "은/는", "와/과", "로/으로" 중 하나를 인자로 받아야 한다.
    // 선택한 DOM을 순환한다.
    this.each(function () {
      ...
      var printJosa = function () {
        var $next = $this.next(), // 선택한 DOM 요소 우측에 .josa 요소가 있는지 확인하는 동시에 조사를 출력하는 컨테이너 요소.
          ...
          hasFinal =
            (/[가-힣]$/.test(str) &&
              (str.substr(-1).charCodeAt(0) - 0xac00) % 28 > 0) ||
            /[가-힣]\d*[013678]$/.test(str) ||
            /[a-z]\d*[1789]$/i.test(str) ||
            /([clmnp]|[blnt](e)|[co](k)|[aeiou](t)|mb|ng|lert)$/i.test(str), // 한글, 숫자, 알파벳 발음에 종성이 있는가?
          josa; // 덧붙일 조사의 최종 값.

        var setJosa = function (josaCase) {
          // josaCase는 출력해야 할 조사 배열.
          // josaInput은 플러그인 사용자로부터 받은 인자.
          if (josaInput === "이/가") {
            // 주격 조사.
            josa = josaCase[0]; // "이(가)", "이", "가"
          } else if (josaInput === "을/를") {
            // 목적격 조사.
            josa = josaCase[1]; // "을(를)", "을", "를"
          } else if (josaInput === "은/는") {
            // 보격 조사.
            josa = josaCase[2]; // "은(는)", "은", "는"
          } else if (josaInput === "와/과") {
            // 접속 조사.
            josa = josaCase[3]; // "와(과)", "과", "와"
          } else if (josaInput === "로/으로") {
            // 부사격 조사.
            josa = josaCase[4]; // "로(으로)", "으로", "로"
          } else {
            // 받아온 josaInput 인자가 없을 때.
            josa = "";
          }
        };

        // 문자열의 특징에 따라서 어떤 조사를 붙여야 할지 분기 처리한다.
        if (!/([가-힣]|\d|[a-z])$/i.test(str)) {
          // 어떤 조사를 붙여야 할지 모르는 경우.
      ...
      if (isInput || isSelect) {
        ...
        $this.on("keyup change", function () {
          // 사용자 이벤트에 의해 실시간으로 값이 변경되면 printJosa 함수를 다시 실행.
          if (isInput) {
            // 인풋 이면.
